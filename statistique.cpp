...
int statistique::Stat_partie2()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from matiere where quantity_matiere BETWEEN '1' AND '100'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int statistique::Stat_partie3()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from matiere where quantity_matiere BETWEEN '101' AND '500'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int statistique::Stat_partie4()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from matiere where quantity_matiere BETWEEN '501' AND '10000'") ;
    while(requete.next())
    {
            count++ ;
    }
